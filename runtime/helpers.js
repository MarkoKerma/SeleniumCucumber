module.exports = {
    ...
    waitUntilAttributeEquals: function (
        ...
        return driver.wait(
            function () {
                ...
                return helpers
                    .getAttributeValueBySelector(elementSelector, attributeName)
                    .then(function (value) {
                        // inspect the value
    ...
    waitForElementToLoad: function (element) {
        return driver.wait(until.elementLocated(element, 20000));
    },

    replaceHTMLCodeFromElement: function (element, attributeName) {
        return driver.findElement(element).then(function (el) {
            return el.getAttribute(attributeName).then(function (elCode) {
                var valueStringed = elCode.replace(/(\r\n|\n|\r)/gm, "");
                var script = `arguments[0].innerHTML='${valueStringed}'`;
                return driver.executeScript(
                    script,
    ...
    replaceHTMLCode: function (htmlCode) {
        return htmlCode.then(function (elCode) {
            var valueStringed = elCode.replace(/(\r\n|\n|\r)/gm, "");
            var script = `arguments[0].innerHTML='${valueStringed}'`;
            return driver.executeScript(
                script,
    ...
    clickWhenClickable: function (locator, timeout) {
        driver.wait(
            function () {
            ...
            timeout,
            "Timeout waiting for " + locator.value
        );
    },

    waitURLToBe: function (url) {
        driver.wait(until.urlIs(url));
    },

    waitUntilAttributeContains: function (
        elementSelector,
        attributeName,
        valuePart
    ) {
        // use either passed in timeout or global default
        var timeout = 10000 || DEFAULT_TIMEOUT;
        ...
        return driver.wait(
            function () {
                ...
                return helpers
                    ...
                    .then(function (value) {
                        // inspect the value
                        return value.includes(valuePart);
                    });
            },
            timeout,
            timeoutMessage
    ...
    getAttributeValueBySelector: function (selector, attributeName) {
        // get the element from the page
        return driver.findElement(selector).then(function (el) {
            return el.getAttribute(attributeName);
        });
    },
};
