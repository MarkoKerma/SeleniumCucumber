...
program
    ...
    .option(
        "-s, --steps <path>",
        "path to step definitions. defaults to " + config.steps,
        config.steps
    )
    .option(
        "-p, --pageObjects <path>",
        "path to page objects. defaults to " + config.pageObjects,
        config.pageObjects
    )
    .option(
        "-o, --sharedObjects [paths]",
        "path to shared objects (repeatable). defaults to " +
    ...
    .option(
        "-b, --browser <path>",
        "name of browser to use. defaults to " + config.browser,
        config.browser
    )
    .option(
        "-k, --browser-teardown <optional>",
        'browser teardown strategy after every scenario (always, clear, none). defaults to "always"',
        config.browserTeardownStrategy
    )
    .option(
        "-r, --reports <path>",
        "output path to save reports. defaults to " + config.reports,
        config.reports
    )
    .option(
    ...
    .option(
        "-j, --junit <path>",
        "output path to save junit-report.xml defaults to " + config.reports
    )
    .option("-t, --tags <tagName>", "name of tag to run", collectPaths, [])
    .option(
        "-f, --featureFiles <paths>",
        "comma-separated list of feature files to run or path to directory defaults to " +
            config.featureFiles,
        config.featureFiles
    )
    .option(
        "-x, --timeOut <n>",
        "steps definition timeout in milliseconds. defaults to " +
            config.timeout,
        coerceInt,
        config.timeout
    )
    .option(
        "-n, --noScreenshot [optional]",
        "disable auto capturing of screenshots when an error is encountered"
    )
    .option(
        "-w, --worldParameters <JSON>",
        "JSON object to pass to cucumber-js world constructor. defaults to empty",
...
program.on("--help", function () {
    console.log(
        "  For more details please visit https://github.com/john-doherty/selenium-cucumber-js#readme\n"
    );
});
...
process.argv.push(
    "json:" +
        path.resolve(__dirname, global.reportsPath, "cucumber-report.json")
);
